...
export function getPcmCaptureWorkletUrl(): string {
  const bytes = new TextEncoder().encode(PCM_CAPTURE_WORKLET_SOURCE);
  let binary = '';
  for (let i = 0; i < bytes.length; i++) binary += String.fromCharCode(bytes[i]!);
  return `data:application/javascript;base64,${btoa(binary)}`;
}

const PCM_CAPTURE_WORKLET_SOURCE = /* js */ `
...
registerProcessor('${PCM_CAPTURE_PROCESSOR_NAME}', VoxflowPcmCaptureProcessor);
`;
